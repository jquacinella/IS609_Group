...
Run a test of the simulation code to make sure it's working
...
```{r ev_heat_map, fig.width=11.5, fig.height=3.5}
...
Since we can't observe which way the receiver is guessing we will use the pearson statistic as a surrogate.
...
We can also look at the p-value of the chi squared statistic, which should correlate:

```{r p-value_heat_map, fig.width=8, fig.height=2.5}
